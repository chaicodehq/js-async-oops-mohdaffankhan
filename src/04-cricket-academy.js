...
export class Batsman extends Player {
  ...
  getStrikeRate() {
    ...
    const totalStrikeRate = this.innings.reduce((sum, inv) => sum + inv.strikeRate, 0);
  ...
  getProfile() {
    return {
      ...
      role: "batsman",
      totalRuns: this.innings.reduce((sum, inv) => sum + inv.runs, 0),
      inningsPlayed: this.innings.length
    };
  }
}
...
export class Bowler extends Player {
  ...
  getEconomy() {
    ...
    const totalEconomy = this.spells.reduce((sum, sp) => sum + sp.economy, 0);
